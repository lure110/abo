...
class Airi_Scripts
{
    ...
    public function enqueue_scripts()
    {
        ...
        $style_depends = array('font-awesome');
        if (class_exists('LAHB', false) && ( Airi()->settings()->get('enable_header_builder', 'no') == 'yes' ||  ( isset($_GET['lastudio_header_builder']) && $_GET['lastudio_header_builder'] == 'inline_mode' ) ) ) {
            wp_enqueue_style('airi-header-builder', get_theme_file_uri( '/assets/css/header-builder.css' ), null, $theme_version);
            $style_depends[] = 'airi-header-builder';
        }

        wp_enqueue_style('airi-theme', get_template_directory_uri() . '/style.css', $style_depends, $theme_version);
        ...
        if (class_exists('LAHB', false) && ( Airi()->settings()->get('enable_header_builder', 'no') == 'yes' ||  ( isset($_GET['lastudio_header_builder']) && $_GET['lastudio_header_builder'] == 'inline_mode' ) ) ) {
            wp_register_script( 'airi-sticky', get_theme_file_uri('/assets/js/plugins/min/jquery.sticky.min.js') , array('jquery'), $theme_version, true);
            wp_enqueue_script('airi-header-builder', get_theme_file_uri( '/assets/header-builder/header-builder.js' ), array('airi-theme'), $theme_version, true);
            $inline_polyfills = 'var doPolyfills=function(){jQuery.getScript(la_theme_config.js_path+la_theme_config.polyfills.css_vars,function(){cssVars()})};"always"===la_theme_config.polyfills.method?doPolyfills():"request"===la_theme_config.polyfills.method&&(window.CSS&&window.CSS.supports&&window.CSS.supports("(--foo: red)")||doPolyfills());';
            wp_add_inline_script( 'airi-header-builder', $inline_polyfills );
        }
