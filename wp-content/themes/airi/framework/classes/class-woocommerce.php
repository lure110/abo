...
if(!class_exists('Airi_WooCommerce')) {

    class Airi_WooCommerce{
        ...
        public function __construct(){
            ...
            add_filter('airi/get_site_layout', array( $this, 'set_site_layout') );

            add_filter('airi/filter/sidebar_primary_name', array( $this, 'set_sidebar_for_shop'), 20 );
            ...
            add_action('woocommerce_after_shop_loop_item_title', array( $this, 'add_count_up_timer_in_product_listing' ), 7 );
            ...
            add_action('airi/action/shop_loop_item_action_top', function(){ echo '<div class="wrap-addto">'; }, 5 );
            add_action('airi/action/shop_loop_item_action_top', array( $this, 'add_quick_view_btn' ), 10 );
            add_action('airi/action/shop_loop_item_action_top', array( $this, 'add_cart_btn' ), 20 );
            add_action('airi/action/shop_loop_item_action_top', array( $this, 'add_wishlist_btn' ), 30 );
            add_action('airi/action/shop_loop_item_action_top', array( $this, 'add_compare_btn' ), 40 );
            add_action('airi/action/shop_loop_item_action_top', function(){ echo '</div>'; }, 50 );


            add_action('woocommerce_before_shop_loop_item_title', array( $this, 'add_count_up_timer_in_product_listing' ), 40 );

            add_action('airi/action/shop_loop_item_action', function(){ echo '<div class="wrap-addto">'; }, 5 );
            add_action('airi/action/shop_loop_item_action', array( $this, 'add_quick_view_btn' ), 10 );
            add_action('airi/action/shop_loop_item_action', array( $this, 'add_cart_btn' ), 20 );
            add_action('airi/action/shop_loop_item_action', array( $this, 'add_wishlist_btn' ), 30 );
            add_action('airi/action/shop_loop_item_action', array( $this, 'add_compare_btn' ), 40 );
            add_action('airi/action/shop_loop_item_action', function(){ echo '</div>'; }, 50 );

            /**
             * Product Page
             */

            $wrap_addto_position = Airi()->settings()->get('product_single_wrap_addto_position', '45');
            $wrap_addto_position = absint($wrap_addto_position);
            if(!in_array($wrap_addto_position, array(45,9,13,29,51,53))){
                $wrap_addto_position = 45;
            }
            ...
            if($wrap_addto_position != 45){
                add_action('woocommerce_single_product_summary', function(){ echo '<div class="clearfix"></div>'; }, $wrap_addto_position);
            }
            add_action('woocommerce_single_product_summary', array( $this, 'add_wishlist_btn' ), $wrap_addto_position);
            add_action('woocommerce_single_product_summary', array( $this, 'add_compare_btn' ), $wrap_addto_position);
            ...
            if( Airi()->settings()->get('product_single_hide_product_title', 'no') == 'yes'){
                remove_action('woocommerce_single_product_summary', 'woocommerce_template_single_title', 5);
            }

            add_filter('template_include', array( $this, 'load_quickview_template'), 20 );

            if( ! airi_string_to_bool(Airi()->settings()->get('move_woo_tabs_to_bottom', 'no')) ){
            ...
            if(Airi()->settings()->get('crosssell_products', 'off') == 'on'){
            ...
            if( Airi()->settings()->get('catalog_mode', 'off') == 'on'){
                // In Loop
                ...
                if( Airi()->settings()->get('catalog_mode_price', 'off') == 'on'){
        ...
        public function wrapper_start(){
            ...
            echo '<div class="row">';
            echo '<main id="site-content" class="'. esc_attr(Airi()->layout()->get_main_content_css_class('col-xs-12 site-content')) .'">';
            echo '<div class="site-content-inner">';
            do_action( 'airi/action/before_render_main_inner' );
        ...
        public function wrapper_end(){
            ...
            do_action( 'airi/action/after_render_main_inner' );
            ...
            echo '</div><!-- ./container -->';
            echo '</div><!-- ./site-main -->';
            do_action( 'airi/action/after_render_main' );
        }
        ...
        public function set_sidebar_for_shop( $sidebar ) {
            ...
            if( in_array( 'is_woocommerce', $context ) ){
                ...
                if(in_array( 'is_archive', $context ) ){
                    ...
                    $sidebar = Airi()->settings()->get('shop_sidebar', $sidebar);

                    if(Airi()->settings()->get('shop_global_sidebar', false)){
                        /*
                    ...
                    if(in_array( 'is_shop', $context)){
                        if( ($single_sidebar = Airi()->settings()->get_post_meta( Airi()->get_page_id(), 'sidebar')) && !empty($single_sidebar) ){
                            $sidebar = $single_sidebar;
                        }
                    }
                    if(in_array( 'is_product_taxonomy', $context)){
                        if( ($tax_sidebar = Airi()->settings()->get_term_meta( get_queried_object_id(), 'sidebar')) && !empty($tax_sidebar) ){
                            $sidebar = $tax_sidebar;
                        }
                    }
                }
                ...
                elseif(in_array('is_product', $context)){
                    $sidebar = Airi()->settings()->get('products_sidebar', $sidebar);

                    if(Airi()->settings()->get('products_global_sidebar', false)){
                        /*
                         * Return global sidebar if option will be enable
                         * We don't need more checking in context
                         */
                    ...
                    if( ($single_sidebar = Airi()->settings()->get_post_meta( get_the_ID(), 'sidebar')) && !empty($single_sidebar) ){
                        $sidebar = $single_sidebar;
                    }
                }
            }
        ...
        public function add_class_to_product_loop($classes, $class, $post_id){
            ...
            if ( $product ) {
                ...
                if( 'on' == Airi()->settings()->get('woocommerce_enable_crossfade_effect') ){
                    $with_second_image = true;
                }

                $disable_second_image = airi_get_wc_loop_prop('disable_alt_image');
                ...
                if($with_second_image && (($galleries = $product->get_gallery_image_ids()) && !empty($galleries[0]))){
                    ...
                    $loop_layout = airi_get_wc_loop_prop('loop_layout');
                    $loop_style = airi_get_wc_loop_prop('loop_style');
                ...
                $enable_rating = Airi()->settings()->get('woocommerce_show_rating_on_catalog', 'off');
                if(get_option( 'woocommerce_enable_review_rating' ) === 'no'){
                    $enable_rating = 'off';
                }
                $classes[] = 'prod-rating-' . esc_attr(Airi()->settings()->get('woocommerce_show_rating_on_catalog', 'off'));
                if( airi_string_to_bool( $enable_rating ) && apply_filters('airi/shop/force_display_rating', true, $product->get_average_rating()) ) {
        ...
        public function add_script_resize_image_in_loop(){
            $image_size = airi_get_wc_loop_prop('image_size');
            if(!empty($image_size)) {
                Airi()->images()->before_resize();
            }
        }
        ...
        public function add_second_thumbnail_to_loop(){
            ...
            $prods_masonry = airi_get_wc_loop_prop('prods_masonry', false);
            ...
            $allow_caching = apply_filters('airi/help/allow_product_image_cache', false);
                ...
                if($prods_masonry){
                    if($image = wp_get_attachment_image_src($product->get_image_id(), $shop_catalog_size)){
                        ...
                        if($use_lazy_load){
                            ...
                            $style .= sprintf('background-image: url(%s);', esc_url(airi_get_relative_url($src)));
                        }
                        ...
                        if($prods_masonry){
                            ...
                            if( ($alt_image_src = wp_get_attachment_image_url($ids[0], $shop_catalog_size, false) ) && $alt_image_src ) {
                                ...
                                if($use_lazy_load){
                                    ...
                                    $style = sprintf('style="background-image: url(%s);"', airi_get_relative_url($alt_image_src));
                                }

                                $html_image_cache .= sprintf(
                                    '<div class="%s" data-background-image="%s"%s></div>',
                                    $elm_css_class,
                                    airi_get_relative_url($alt_image_src),
                                    $style
                                );
                            }
                        }
            ...
            echo airi_render_variable($html_image_cache);
        }
        ...
        public function add_quick_view_btn(){
            if( 'on' == Airi()->settings()->get('woocommerce_show_quickview_btn', 'off') ){
                global $product;
                printf(
                    ...
                    esc_attr_x('Quick Shop', 'front-view', 'airi'),
                    esc_attr_x('Quick Shop', 'front-view', 'airi')
                );
            }
        }
        ...
        public function add_cart_btn(){
            if( Airi()->settings()->get('catalog_mode', 'off') != 'on' && Airi()->settings()->get('woocommerce_show_addcart_btn', 'on') == 'on' ) {
        ...
        public function add_compare_btn(){
            ...
            if( Airi()->settings()->get('woocommerce_show_compare_btn', 'off') == 'on' ) {
                if ( !empty($yith_woocompare->obj) ) {
                    ...
                    printf(
                        ...
                        esc_attr($css_class),
                        esc_url($url),
                        esc_attr_x('Add to Compare','front-view', 'airi'),
                    ...
                    $url = '#';
                    $text = esc_html_x('Add to Compare','front-view', 'airi');
                    printf(
                        '<a class="%s" href="%s" title="%s" rel="nofollow" data-product_title="%s" data-product_id="%s">%s</a>',
                        esc_attr($css_class),
                        esc_url($url),
        ...
        public function add_wishlist_btn(){
            ...
            if(Airi()->settings()->get('woocommerce_show_wishlist_btn', 'off') == 'on'){
                global $product;
                if (function_exists('YITH_WCWL')) {
                    ...
                    if (YITH_WCWL()->is_product_in_wishlist($product->get_id(), $default_wishlist)) {
                        $text = esc_html_x('View Wishlist', 'front-view', 'airi');
                        $class = 'add_wishlist la-yith-wishlist button added';
                        $url = YITH_WCWL()->get_wishlist_url('');
                    }
                    else {
                        $text = esc_html_x('Add to Wishlist', 'front-view', 'airi');
                ...
                elseif(class_exists('TInvWL_Public_AddToWishlist')){
                    ...
                    if(Airi_WooCommerce_Wishlist::is_product_in_wishlist($product->get_id())){
                        $class = 'add_wishlist button la-core-wishlist added';
                        $url = airi_get_wishlist_url();
                        $text = esc_html_x('View Wishlist', 'front-view', 'airi');
                    }
                    else{
        ...
        public function add_category_in_product_listing(){
            global $product;
            add_filter('get_the_terms', 'airi_exclude_demo_term_in_category');
            echo wc_get_product_category_list($product->get_id(),'<span>, </span>', '<div class="product_item--category-link">', '</div>');
            airi_deactive_filter('get_the_terms', 'airi_exclude_demo_term_in_category');
        }
        ...
        public function shop_loop_item_excerpt(){
            ...
            $is_main_loop = airi_get_wc_loop_prop('is_main_loop', false);
            $loop_layout = airi_get_wc_loop_prop('loop_layout', 'grid');
        ...
        public static function product_per_page(){
            return apply_filters('airi/filter/product_per_page', Airi()->settings()->get('product_per_page_default', 9));
        }

        public static function product_per_page_array(){
            $per_page_array = apply_filters('airi/filter/product_per_page_array', Airi()->settings()->get('product_per_page_allow', ''));
        ...
        public function set_cookie_default(){
            if (isset($_GET['per_page']) && $per_page = $_GET['per_page']) {
                add_filter('airi/filter/product_per_page', array( $this, 'get_parameter_per_page'));
            }
        }
        ...
        public function add_count_up_timer_to_single(){
            if(!isset($_GET['product_quickview']) && Airi()->settings()->get('show_product_countdown')){
                global $product;
                $sale_price_dates_to = $product->get_date_on_sale_to() && ( $date = $product->get_date_on_sale_to()->getOffsetTimestamp() ) ? date( 'Y/m/d H:i:s', $date ) : '';
                if(!empty($sale_price_dates_to)){
        ...
        public function check_condition_show_upsell_crosssel(){
            if ( Airi()->settings()->get('related_products', 'off') != 'on' ) {
                remove_action('woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20);
            }
            if ( Airi()->settings()->get('upsell_products', 'off') != 'on' ) {
                remove_action('woocommerce_after_single_product_summary', 'woocommerce_upsell_display', 15);
            }
        }

        public function add_custom_tab($tabs){
            if ( Airi()->settings()->get('woo_enable_custom_tab', 'off') == 'on' ) {
                $tab_content = Airi()->settings()->get('woo_custom_tab_content', '');
                $tab_title = Airi()->settings()->get('woo_custom_tab_title', '');
        ...
        public function get_custom_tab_content(){
            echo Airi_Helper::remove_js_autop( Airi()->settings()->get('woo_custom_tab_content', ''), true);
        }
        ...
        public function modify_availability_text_on_product_page( $availability, $product ){
            if ( ! $product->is_in_stock() ) {
                ...
                $availability = $availability = __( 'in stock', 'airi' );
            }
            return $availability;
        }
        ...
        public function add_sku_to_single_product(){
            ...
            if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ){
                ...
                <div class="product_meta-top">
                    <span class="sku_wrapper"><span class="sku"><?php if($sku = $product->get_sku()) { echo airi_render_variable($sku); } else { esc_html__( 'N/A', 'airi' ); } ?></span></span>
                </div>
                <?php
            }
        }
        ...
        public function modify_ajax_cart_fragments( $fragments ){
            ...
            $text = '<span class="la-cart-text">'. esc_html_x('%s items','front-view', 'airi') .'</span>';
        ...
        public function add_coupon_form_into_cart(){
            if ( wc_coupons_enabled() ) : ?>
                <div class="la-coupon-form">
                    <h2><?php echo esc_html_x('Coupon Code', 'front-view', 'airi') ?></h2>
                    <p><?php echo esc_html_x('Enter your coupon code if you have one.','front-view', 'airi')?></p>
                    <div class="la-coupon">
                        <p class="form-row form-row-wide">
                            <input type="text" class="input-text" id="coupon_code_ref" value="" placeholder="<?php echo esc_attr_x( 'Enter your coupon code..', 'front-view', 'airi' ); ?>" />
                        </p>
                        <button type="button" class="button" id="coupon_btn_ref"><?php echo esc_html_x( 'Apply coupon', 'front-view', 'airi' ); ?></button>
                    </div>
                </div>
            <?php endif;
        }
        ...
        public function add_more_button_to_cart_from(){
            ...
            ?>
            <input type="submit" class="button btn-clear-cart" name="clear-cart" value="<?php echo esc_attr_x('Clear Cart', 'front-view', 'airi');?>">
            <a href="<?php echo esc_url(!empty($category_recent_link) ? $category_recent_link : wc_get_page_permalink('shop')) ?>" class="btn"><?php echo esc_html_x('Continue Shopping', 'front-view', 'airi'); ?></a>
            <?php
        }
        ...
        public function woocommerce_share(){
            if(Airi()->settings()->get('product_sharing') == 'on'){
                ...
                echo sprintf( '<label>%s</label>', esc_html_x('Share with', 'front-end', 'airi') );
                airi_social_sharing($post_link,$post_title,$image);
                echo '</div>';
            }
        }
        ...
        public function woocommerce_before_shipping_calculator(){
            printf(
                ...
                esc_html_x('Calculate Shipping', 'front-view', 'airi'),
                esc_html_x('Estimate your shipping fee *', 'front-view', 'airi')
            );
        }
