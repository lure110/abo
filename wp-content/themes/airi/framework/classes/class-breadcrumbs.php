...
class Airi_Breadcrumbs {
	...
	function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'page_title_format' => '<header><h1 class="page-title">%s</h1></header>',
			...
			'labels'            => array(),
			'date_labels'       => array(),
			'action'            => 'airi/action/breadcrumbs/render_html',
			'css_namespace'     => array(),
			'path_type'         => 'default', // minified , full, default
			'post_taxonomy'     => apply_filters(
				'airi/filter/breadcrumbs/trail_taxonomies',
				array(
					'post'      => 'category',
					'product'	=> 'product_cat'
				)
		...
		$this->args = apply_filters( 'airi/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'airi/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
		...
		do_action('airi/action/breadcrumbs/before_render_html', $this, $this->args);

		if ( true === $this->args['echo'] ) {
			echo airi_render_variable($result);
		}
		else {
			return $result;
		}
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'airi/filter/breadcrumbs/trail', $breadcrumb, $this->args );
	...
	public function get_title() {
		...
		if(is_singular() && !is_page()){
			if(!is_singular(array('la_portfolio','la_team_member'))){
				$this->args['page_title_format'] = '<header><div class="page-title h1">%s</div></header>';
			}
		}

		$title = apply_filters(
			'airi/filter/page_title',
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html_x( 'Browse:', 'front-view', 'airi' ),
			'home'                => $this->home_title(),
			'error_404'           => esc_html_x( '404 Not Found', 'front-view', 'airi' ),
			'archives'            => esc_html_x( 'Archives', 'front-view', 'airi' ),
			'search'              => esc_html_x( 'Search results for &#8220;%s&#8221;', 'front-view', 'airi' ),
			'paged'               => esc_html_x( 'Page %s', 'front-view', 'airi' ),
			'archive_minute'      => esc_html_x( 'Minute %s', 'front-view', 'airi' ),
			'archive_week'        => esc_html_x( 'Week %s', 'front-view', 'airi' ),
	...
	public function home_title() {
		...
		$title            = esc_html_x( 'Home', 'front-view', 'airi' );
		$use_custom_title = apply_filters( 'airi/filter/breadcrumbs/custom_home_title', true );
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'airi/filter/breadcrumbs/custom_trail', false, $this->args );
		...
		if ( is_front_page() ) {
			...
			elseif ( is_archive() ) {
				...
				if ( is_post_type_archive() ) {
					...
					if(is_search()){
						$this->_page_title = sprintf( $this->args['labels']['search'], get_search_query() );
					}
		 ...
		 */
		$this->items = apply_filters( 'airi/filter/breadcrumbs/items', $this->items, $this->args, $this );
	...
	public function _add_item( $format = 'link_format', $label, $url = '', $class = '', $prepend = false, $is_page_title = false ) {
		...
		if ( true === $prepend ) {
			...
			array_unshift( $this->crumbs, $tmp_crumbs );
	...
	public function add_blog_page() {
		...
		if ( is_object($post) &&  0 < $post->post_parent ) {
	...
	public function add_post_terms( $post_id ) {
		...
		if ( ! empty( $this->args['post_taxonomy'][ $post_type ] ) ) {
			add_filter('get_object_terms', 'airi_exclude_demo_term_in_category');
			$post_terms = wp_get_post_terms( $post_id, $this->args['post_taxonomy'][ $post_type ] );
			airi_deactive_filter('get_object_terms', 'airi_exclude_demo_term_in_category');
	...
	public function add_post_type_archive_items() {
		...
		if ( is_paged() ) {
			...
			$label = ! empty( $post_type_object->labels->archive_title ) ? $post_type_object->labels->archive_title : post_type_archive_title( '', false );
	...
	public function add_post_hierarchy( $post_id ) {
		...
		if ( 'post' === $post_type ) {
			...
			$this->add_rewrite_front_items();
			if( 'default' != $this->args['path_type'] ){
	 ...
	 */
	public function _process_single_tag( $tag, $post_id ) {
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'airi/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'airi/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
	...
	public function get_breadcrumb() {
		return apply_filters( 'airi/filter/breadcrumbs/get_breadcrumb', $this->crumbs, $this );
	}
}