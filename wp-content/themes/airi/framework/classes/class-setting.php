...
class Airi_Setting{
    ...
    public function __construct( $args = array() ) {
        $this->args = array_merge(array(
            'option_name'       => Airi::get_option_name(),
            'post_meta_name'    => Airi::get_original_option_name(),
            'term_meta_name'    => Airi::get_original_option_name()
        ), $args);
    }
    ...
    public function get( $key = '', $default = '' ){
        ...
        return apply_filters("airi/setting/option/get_single", $value, $key );
    }
    ...
    public function get_setting_by_context( $key, $default = '', $context = array()){
        ...
        if(in_array('is_home', $context)){
            ...
            if(!empty($_value)){
                if(is_array($_value)){
                    if(Airi_Helper::is_not_empty_array_ref($_value)){
                        $value = $_value;
                    }
                }else{
                    if($_value !== 'inherit'){
        ...
        elseif(in_array('is_archive', $context)){
            ...
            if(in_array('is_shop', $context)){
                ...
                if(Airi()->get_page_id()){
                    $_value = $this->get_post_meta( Airi()->get_page_id(), $key);
                    if(!empty($_value)){
                        if(is_array($_value)){
                            if(Airi_Helper::is_not_empty_array_ref($_value)){
                                $value = $_value;
        ...
        return apply_filters('airi/setting/get_setting_by_context', $value, $key, $context);
    }
