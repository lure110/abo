...
<div id="la_compare_table_wrapper">
    ...
    $lists = Airi_WooCommerce_Compare::get_data();

    $default_attr = airi_get_wc_attribute_for_compare();
    $tax_attr = airi_get_wc_attribute_taxonomies();
    $all_attr = array_merge($default_attr, $tax_attr);

    $attribute_allow = Airi()->settings()->get('compare_attribute', $default_attr);
    ...
    if (!empty($lists)) {
        foreach ($lists as $product_id) {
            ...
            $term['id'] = $product_id;
            $term['sku'] = ($_product->get_sku() ? $_product->get_sku() : __('N/A', 'airi'));
        ...
        <table class="la-compare-table">
            ...
            foreach( $attribute_allow as $attr_alow ){
                if(in_array($attr_alow, array('image','title','add-to-cart'))){
                    $label_list['info'] = esc_html_x('Product Info', 'front-view', 'airi');
                }
                else{
                    ...
                    elseif ($k == 'stock') {
                        ...
                        if ($item['availability'] && !empty($item['availability']['availability'])) {
                            ...
                            if ($item['stock']) {
                                ...
                                $text_stock = __('Out of stock', 'airi');
        ...
        ?>
        <p class="text-center"><?php esc_html_e('No products were added to the list', 'airi') ?></p>
        <?php
    }
    ?>
</div>
