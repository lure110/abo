...
function lahb_search( $atts, $uniqid, $once_run_flag = true ) {
    ...
    if( $show_category_dropdown == 'true' ) {
        $cat_dropdown_args = array(
            'show_option_all'    => esc_html__('All Categories', 'lastudio-header-builder'),
            'name'               => 'cat',
    ...
    $extra_html .= '<button type="reset" class="search-button search-reset"><i class="dl-icon-close"></i></button><button class="search-button" type="submit"><i class="'.esc_attr($search_icon).'"></i></button>';
