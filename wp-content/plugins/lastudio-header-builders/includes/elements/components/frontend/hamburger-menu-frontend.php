...
function lahb_hamburger_menu( $atts, $uniqid, $once_run_flag = true ) {
	...
	if(!empty($image_logo) && class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'photon' )){
	...
	if ( $once_run_flag ) :
        ...
        $css_el_hm_box = 'body ' . $elm_uniqid . ', body .la-hamburger-wrap' . $elm_uniqid;
        ...
        $css_el_hm_menu_item_shape = '.body '. $elm_uniqid .' .hamburger-nav > li > a .hamburger-nav-icon';
        $css_el_hm_menu_item_shape_hover = 'body '. $elm_uniqid .' .hamburger-nav > li:hover > a .hamburger-nav-icon';
	...
	if ( $once_run_flag ) {
        if ( $hamburger_type == 'full' ) {
			...
			<div class="hamburger-full-wrap">
			    <a href="javascript:;" class="btn-close-hamburger-menu-full"><i class="dl-icon-close"></i></a>
        ...
        elseif ($hamburger_type == 'toggle') {
            $out .= '<div class="lahb-element--dontcopy hamburger-menu-wrap la-hamuburger-bg hamburger-menu-content ' . esc_attr($menu_style) . ' la-hamburger-wrap-' . esc_attr($uniqid) . ' ' . $toggle_from . ' ' . esc_attr($extra_class_panel) . '">
			    <a href="javascript:;" class="btn-close-hamburger-menu"><i class="dl-icon-close"></i></a>
            ...
            if ($search == 'true') :
                $out .= '<form role="search" class="search-form" action="' . esc_url(home_url('/')) . '" method="get"><input name="s" type="text" class="search-field hamburger-search-text-box" placeholder="' . ( !empty($placeholder) ? LAHB_Helper::translate_string($placeholder, $com_uniqid) : '' ) . '"><button class="search-button" type="submit"><i class="dl-icon-search4"></i></button></form>';
            endif;
