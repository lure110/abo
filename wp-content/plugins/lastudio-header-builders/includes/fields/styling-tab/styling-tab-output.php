...
function lahb_styling_tab_output( $atts = array(), $el = '', $class = '', $class_hover = '' ) {
	...
	if ( is_array( $atts ) ) {
        foreach ($atts as $var_name => $var_val) {
            ...
            // desktop
            if (strpos($var_name, 'sae_' . $el) !== false) {
                $var_name = str_replace('sae_' . $el, 'all', $var_name);
                $variables[$var_name] = $var_val;
            } // tablets
            elseif (strpos($var_name, 'ste_' . $el) !== false) {
                $var_name = str_replace('ste_' . $el, 'tablets', $var_name);
                $variables[$var_name] = $var_val;
            } // mobiles
            elseif (strpos($var_name, 'sme_' . $el) !== false) {
                $var_name = str_replace('sme_' . $el, 'mobiles', $var_name);
                $variables[$var_name] = $var_val;
            }
        }
    }
